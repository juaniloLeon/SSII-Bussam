	...
	<div class="row">
		...
		<div class="col-lg-8 col-md-10 bg-info">
			...
			<h3>
				...
				<%
				/*AQUI QUIERO IMPRIMIR LINEA, QUE ES LA VARIABLE QUE INDICA A QUE LINEA SE HACE LA QUERY*/
			%>
			</h3>
			<table class="table table-striped">
				<tr>
					<th style="width: 20%;">Numero de bonobus</th>
					<th style="width: 20%;">Saldo actual</th>

				</tr>
				<%
				String bono = request.getParameter("bonobus"); 

					String query = "select idbonobus, re-(va*0.7) from bonobus left join (select sum(cantidad) re, bonobus_idbonobus from recargas group by bonobus_idbonobus) r on (idbonobus=r.bonobus_idbonobus) join (select count(idvalidaciones) va, bonobus_idbonobus from validaciones group by bonobus_idbonobus) v on (idbonobus=v.bonobus_idbonobus) where idbonobus="+bono;
					beanDB basededatos = new beanDB();
					
					try{String[][] tablares = basededatos.resConsultaSelectA3(query);

					ArrayList<Bonos> listagrupos = new ArrayList<Bonos>();
					for (int i = 0; i < tablares.length; i++) {
						listagrupos.add(new Bonos(Integer.parseInt(tablares[i][0]), tablares[i][1]));
					}
				 
					for (Bonos g : listagrupos) { //g es una variable tipo grupo que va recorriendo la lista
				%><tr>

					<td style="width: 20%;"><%=g.getID()%></td>
					<td style="width: 20%;"><%=g.getSaldo()%>€</td>
				...
				<tr>
					...
					<td style="width: 20%;"> ESTE BONO</td>
					<td style="width: 20%;">NO FIGURA EN LA BASE DE DATOS</td>
