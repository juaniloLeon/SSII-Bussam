...
-->
			...
			<table class="table table-striped">
				<tr>
					<th style="width: 20%;">Número de Coche</th>
				...
				<%
					String query = "select * from autobus order by idautobus;";
					beanDB basededatos = new beanDB();
					String[][] tablares = basededatos.resConsultaLineas(query);

					ArrayList<Buses> listagrupos = new ArrayList<Buses>();
					for (int i = 0; i < tablares.length; i++) {
						listagrupos.add(new Buses(Integer.parseInt(tablares[i][0]), tablares[i][1]));
					}
				%>

				<%
					for (Buses g : listagrupos) { //g es una variable tipo grupo que va recorriendo la lista
				%><tr>

					<td style="width: 20%;"><%=g.getID()%></td>
					<td style="width: 20%;"><%=g.getMatricula()%></td>
